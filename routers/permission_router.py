...
from dto.permission_dto import PermissionResponse, CreatePermissionCommand, UpdatePermissionByIdCommand, \
    UpdatePermissionByUserIdOrgIdCommand
...
class PermissionRouter(PermissionService):
    ...
    async def update(self, id:str,command: UpdatePermissionByIdCommand) -> PermissionResponse:
        return await self.update_permission_by_id(id, command)

    @permission_router.put(base_path + "/update", response_model=PermissionResponse)
    async def update_by_user_id_org_id(self, command: UpdatePermissionByUserIdOrgIdCommand) -> PermissionResponse:
        return await self.update_permission_by_user_id_org_id(command)
